...
exports.epartment_Head_Show_Assign_work_Employee = async (req, res) => {
    try {
        ...
        const assignWorkList = await AssignWork.find({ whoAssignWorkId: userId })
            .populate("workAssignToId", "empName empEmail empMobile").populate("productId", "productName modelNo partNo TacNo productType");
...
function generateNextSlNo(lastSlNo) {
    const now = new Date();
...
exports.getNextFirmwareSlNo = async (req, res) => {
    try {
        const lastFirmware = await FirmWareModel
            .findOne({ slNo: { $regex: "^TIA/" } })
            .sort({ createdAt: -1 })
        ...
        const nextSlNo = generateNextSlNo(lastFirmware?.slNo);
        ...
        return res.status(500).json({
            success: false,
            message: "Error fetching next firmware slNo"
        });
    }
};
